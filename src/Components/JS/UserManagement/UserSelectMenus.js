...
export default function UserSelectMenus(props) {
    ...
    useEffect(() => {
        ...
        if (props.selectedUser.length === 1) {
            fetch(`https://drf-react-ticketing-backend.herokuapp.com/api/project-users/${props.selectedUser}`, requestOptions)
    ...
    useEffect(() => {
        ...
        fetch('https://drf-react-ticketing-backend.herokuapp.com/api/projects/', requestOptions)
            .then(response => response.json())
            .then(data => {
